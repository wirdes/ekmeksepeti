...
const AddAddress = props => {
    ...
    const updateAddress = async navigation => {
        ...
        try {
            ...
            Toast.show({
                type: 'success',
                text1: 'Güncellendi',
                text2: 'Adresiniz Güncellendi. Anasayfaya Sayfasına yönlendiriliyorsunuz',
            });
    ...
    return ( <
        ...
        <
        Text style = {
            { fontSize: 25, color: appStyle.color } } >
        Adres Ekleme veya Değiştirme { ' ' } <
        /Text>{' '} <
        /View>{' '} <
        View style = { style.form } >
        <
        TextInput multiline style = { style.input }
        autoCapitalize = "none"
        ...
        onPress = {
            () => updateAddress(props.navigation) }
        style = { style.button } >
        <
        Text style = { style.buttonText } > { ' ' } {!isLoading ? 'Kaydet' : 'Kayıt ediliyor...' } { ' ' } <
        /Text>{' '} <
        /TouchableOpacity>{' '} <
        /View>{' '} <
        /View>
